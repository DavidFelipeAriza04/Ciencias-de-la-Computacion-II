import { Component, AfterViewInit, Output, effect } from '@angular/core';
...
export class MapScreenComponent implements AfterViewInit {
  ...
  private drawLines():void{
    ...
    this.municipios.forEach((municipio) => {
        for (let i = contador; i < 30; i++) {
            if (municipio[this.municipios[i]["nombre"].replaceAll(" ", "") + "Conexion"] != 0) {
  ...
  private dibujarRecorridoEffect() {
    effect(() => {
  ...
  private dibujarRecorrido(){ 
    if(this.mapaService.get_recorrido.length == 0) return;
    
    for (let i = 0; i < this.mapaService.get_recorrido.length; i++) {
      if (i != this.mapaService.get_recorrido.length - 1) {
          let municipio1:Municipio | undefined = this.mapaService.municipios_sevice
          .find(municipio => municipio["nombre"] === this.mapaService.recorrido()[i].replaceAll("_", " "));
          
          let municipio2:Municipio | undefined = this.mapaService.municipios_sevice
          .find(municipio => municipio["nombre"] === this.mapaService.recorrido()[i + 1].replaceAll("_", " "));
